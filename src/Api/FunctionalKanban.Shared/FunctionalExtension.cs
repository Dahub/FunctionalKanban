namespace FunctionalKanban.Shared
{
    ...
    {
        public static Option<T> CastTo<T, R>(this Option<R> value) => value.Bind<R, T>((x) => x is T t ? t : None);

        public static Exceptional<IEnumerable<T>> ToExceptionalOfList<T>(this IEnumerable<Exceptional<T>> exceptionals) =>
        ...
        public static Validation<IEnumerable<T>> ToValidationOfList<T>(this IEnumerable<Validation<T>> validations) =>
